...
import { Wallet, RefreshCw, Copy, Plus, Check } from 'lucide-react';
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
...
export const Header: React.FC = () => {
  ...
  const refetch = () => {
    ...
    if (timeSinceLastFetch < FETCH_COOLDOWN) {
      ...
      console.log(`Balance refresh on cooldown. Please wait ${Math.ceil((FETCH_COOLDOWN - timeSinceLastFetch) / 1000)} more seconds.`);
      return;
    }
  ...
  const handleLogin = async (): Promise<void> => {
    try { await login(); } catch (error) { console.error('Failed to login', error); }
  };

  const handleLogout = async (): Promise<void> => {
    try { await logout(); } catch (error) { console.error('Failed to logout', error); }
  };
